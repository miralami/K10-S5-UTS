...
if ($SingleTerminal) {
    ...
    $commands = @(
        ...
        "`"cd chat-service && npm run server:start`""
    )
    ...
    Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$RootDir\chat-service'; Write-Host 'Chat Service' -ForegroundColor Yellow; npm run server:start"
