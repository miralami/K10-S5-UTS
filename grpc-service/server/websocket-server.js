...
class UserManager {
  ...
  removeUser(ws) {
    ...
    if (userData && userData.ws === ws) {
      ...
      console.log(`User ${userData.user.name} (${userId}) disconnected. Total users: ${this.users.size}`);
    ...
    try {
      ...
      if (parsed.type === 'message') {
        ...
        if (recipientId) {
          ...
          console.log(`Private message from ${sender.name} to ${recipient.name}: ${text.trim()}`);
        } else {
